...
This guide will explain the specification of the files used by the games, i.e. the *source files* used to make the Data File (which is in turn read by the game).
...
Concretely, the files can be split in two categories : 
- "Media files", mainly images and sounds
- "Descriptor files", text files with a ".dat" extension that contain information following a [defined syntax](../internal/ressource%20file%20format/0.3.4.md). 
